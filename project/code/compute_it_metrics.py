...
for i in range(spike.shape[1]):
    ...
    print(i)
    mi = mutual_info_classif(X=np.array(batch, dtype=np.int), y=np.array(spike[:, i], dtype=np.int), discrete_features=True)
    mi_array[i] = mi
    # print(mi)

for i in range(mi_array.shape[0]):
    if np.sum(mi_array[i, :]) > 0:
        print(i, np.sum(mi_array[i, :]))